...
-- 네이버 쇼핑몰 분석 도구 데이터베이스 스키마 재구성
-- 생성일: 2025-06-14

-- 1. 기존 테이블이 있다면 삭제 (주의: 데이터 손실 위험)
DROP TABLE IF EXISTS public.category_uploads CASCADE;
DROP TABLE IF EXISTS public.naver_categories CASCADE;
DROP TABLE IF EXISTS public.keyword_analysis CASCADE;
DROP TABLE IF EXISTS public.store_analysis CASCADE;
...
-- 6. 스토어 분석 결과 테이블 생성
CREATE TABLE public.store_analysis (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_keyword_analysis_user_id ON public.keyword_analysis(user_id);
CREATE INDEX idx_store_analysis_user_id ON public.store_analysis(user_id);
CREATE INDEX idx_category_uploads_user_id ON public.category_uploads(uploaded_by);
...
-- naver_categories 테이블 정책 (모든 인증된 사용자가 읽기 가능)
