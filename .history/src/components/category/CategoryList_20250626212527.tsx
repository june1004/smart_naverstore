        ...
        <div className="space-y-4">
          ...
          {selectedLargeCategory && !selectedMediumCategory && (
            <div className="flex flex-wrap gap-2 mt-2 max-h-[300px] overflow-y-auto">
              ...
              {Array.from(new Set(all.filter(c => matchCategory(c.large_category, selectedLargeCategory) && c.medium_category).map(c => c.medium_category)))
                .filter(Boolean)
                .map(medium => (
                  <Button
                    key={medium}
          ...
          {selectedLargeCategory && selectedMediumCategory && !selectedSmallCategory && (
            <div className="flex flex-wrap gap-2 mt-2 max-h-[300px] overflow-y-auto">
              ...
              {Array.from(new Set(all.filter(c => matchCategory(c.large_category, selectedLargeCategory) && c.medium_category === selectedMediumCategory && c.small_category).map(c => c.small_category)))
                .filter(Boolean)
                .map(small => (
                  <Button
                    key={small}
          ...
          {selectedLargeCategory && selectedMediumCategory && selectedSmallCategory && (
            <div className="flex flex-wrap gap-2 mt-2 max-h-[300px] overflow-y-auto">
              ...
              {Array.from(new Set(all.filter(c => matchCategory(parseCategoryPathParts(c.category_path).large, selectedLargeCategory) && matchCategory(parseCategoryPathParts(c.category_path).medium, selectedMediumCategory) && matchCategory(parseCategoryPathParts(c.category_path).small, selectedSmallCategory) && c.smallest_category).map(c => c.smallest_category)))
                .filter(Boolean)
                .map(smallest => (
                  <Button
                    key={smallest}
