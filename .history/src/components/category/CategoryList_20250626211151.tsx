...
const CategoryList = ({ selectedLevel, onLevelFilter, refetchRef }: CategoryListProps) => {
  ...
  const largeCategories = Array.from(
    new Map(
      all
        ...
        .map(c => [normalize(c.large_category), c])
    ).values()
  );
  ...
  // 대분류명 매칭 함수 (normalize 사용, 후보군 확장)
  ...
  if (selectedLargeCategory && !selectedMediumCategory && !selectedSmallCategory) {
    filtered = all.filter(c => {
      // 대분류 후보군에서 normalize로 비교
      const parts = parseCategoryPathParts(c.category_path);
      return matchLargeCategory(parts.large, selectedLargeCategory);
    });
    ...
    filtered = all.filter(c => {
      const parts = parseCategoryPathParts(c.category_path);
      return matchLargeCategory(parts.large, selectedLargeCategory) && matchLargeCategory(parts.medium, selectedMediumCategory);
    });
    ...
    filtered = all.filter(c => {
      const parts = parseCategoryPathParts(c.category_path);
      return matchLargeCategory(parts.large, selectedLargeCategory) && matchLargeCategory(parts.medium, selectedMediumCategory) && matchLargeCategory(parts.small, selectedSmallCategory);
    });
        ...
        <div className="space-y-4">
          ...
          {selectedLargeCategory && selectedMediumCategory && selectedSmallCategory && (
            <div className="flex flex-wrap gap-2 mt-2 max-h-[300px] overflow-y-auto">
              ...
              {Array.from(new Set(all.filter(c => matchLargeCategory(parseCategoryPathParts(c.category_path).large, selectedLargeCategory) && matchLargeCategory(parseCategoryPathParts(c.category_path).medium, selectedMediumCategory) && matchLargeCategory(parseCategoryPathParts(c.category_path).small, selectedSmallCategory) && c.smallest_category).map(c => c.smallest_category)))
                .filter(Boolean)
                .map(smallest => (
                  <Button
                    key={smallest}
