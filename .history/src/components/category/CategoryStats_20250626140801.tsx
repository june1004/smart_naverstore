    ...
    queryFn: async () => {
      ...
      try {
        ...
        const { data: allCategories, error: categoriesError } = await supabase
          .from('naver_categories')
          .select('category_path, category_level, large_category')
          .eq('is_active', true);
        ...
        const largeCounts = NAVER_LARGE_CATEGORIES.map(name => {
          const count = allCategories?.filter(c => c.category_level === 1 && matchLargeCategory(c.large_category, name)).length || 0;
          return { name, count };
        });
        // 기존 집계도 유지
