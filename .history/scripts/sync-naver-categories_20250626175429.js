...
async function syncToSupabase(categories) {
  ...
  if (!Array.isArray(categories) || categories.length === 0) {
    console.log('업데이트할 카테고리 데이터가 없습니다.');
    return;
  }
  // Supabase REST API: 최대 1000개씩 배치로 upsert
  const batchSize = 1000;
  for (let i = 0; i < categories.length; i += batchSize) {
    const batch = categories.slice(i, i + batchSize);
    const res = await fetch(`${SUPABASE_URL}/rest/v1/naver_categories?on_conflict=category_id`, {
      method: 'POST',
      headers: {
    ...
    console.log(`Supabase upsert: ${i + 1} ~ ${i + batch.length} / ${categories.length}`);
  }
  console.log('Supabase 카테고리 동기화 완료!');
}
